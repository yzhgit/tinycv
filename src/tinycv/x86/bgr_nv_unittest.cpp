...
template<>
void I4202NV21<uint8_t>(int32_t height,
                        int32_t width,
                        int32_t inStrideY,
...
template<>
void I4202NV12<uint8_t>(int32_t height,
                        int32_t width,
                        int32_t inStrideY,
                        const uint8_t *inDataY,
                        int32_t inStrideU,
...
template<>
void NV212I420<uint8_t>(int32_t height,
                        int32_t width,
                        int32_t inStrideY,
                        const uint8_t *inDataY,
                        int32_t inStrideVU,
...
template<>
void NV122I420<uint8_t>(int32_t height,
                        int32_t width,
                        int32_t inStrideY,
                        const uint8_t *inDataY,
                        int32_t inStrideUV,
...
void Color2NVTest(int32_t height, int32_t width)
{
    ...
    if (mode == RGB2NV12_MODE) {
        ...
        tinycv::RGB2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        tinycv::I4202NV12<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == RGB2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_RGB2YUV_I420);
        tinycv::RGB2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        tinycv::I4202NV21<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV12_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        tinycv::BGR2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        tinycv::I4202NV12<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        tinycv::BGR2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        tinycv::I4202NV21<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    }
...
void Color2NVMultiPlaneTest(int32_t height, int32_t width)
{
    ...
    if (mode == RGB2NV12_MODE) {
        ...
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_RGB2YUV_I420);
        tinycv::RGB2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        tinycv::I4202NV21<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV12_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        tinycv::BGR2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        tinycv::I4202NV12<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        tinycv::BGR2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        tinycv::I4202NV21<uint8_t>(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    }
    checkResult<uint8_t, 1>(dst.get(), dst_ref.get(), 3 * height / 2, width, width, width, 1.01f);
}
...
void NV2ColorTest(int32_t height, int32_t width)
{
    ...
    if (mode == NV122RGB_MODE) {
        tinycv::NV122RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV12);
    } else if (mode == NV122BGR_MODE) {
        tinycv::NV122BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_NV12);
    } else if (mode == NV212RGB_MODE) {
        tinycv::NV212RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV21);
    } else if (mode == NV212BGR_MODE) {
        tinycv::NV212BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
...
void NV2ColorMultiPlaneTest(int32_t height, int32_t width)
{
    ...
    cv::Mat dstMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 3), dst_ref.get());
    if (mode == NV122RGB_MODE) {
        tinycv::NV122RGB<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV12);
    } else if (mode == NV122BGR_MODE) {
        tinycv::NV122BGR<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_NV12);
    } else if (mode == NV212RGB_MODE) {
        tinycv::NV212RGB<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV21);
    } else if (mode == NV212BGR_MODE) {
        tinycv::NV212BGR<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_NV21);
    }
    checkResult<uint8_t, 3>(dst.get(), dst_ref.get(), height, width, 3 * width, 3 * width, 1.01f);
}
