...
void Color2YUV420MultiPlaneTest(int32_t height, int32_t width)
{
    ...
    cv::Mat dstMat(3 * height / 2, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 1), dst_ref.get());
    if (mode == RGB2I420_MODE) {
        tinycv::RGB2I420<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width / 2, dst.get() + height * width, width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat, cv::COLOR_RGB2YUV_I420);
    } else if (mode == BGR2I420_MODE) {
        tinycv::BGR2I420<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width / 2, dst.get() + height * width, width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat, cv::COLOR_BGR2YUV_I420);
    }
    checkResult<uint8_t, 1>(dst.get(), dst_ref.get(), 3 * height / 2, width, width, width, 1.01f);
}
...
void ColorAlpha2YUV420Test(int32_t height, int32_t width)
{
    ...
    std::unique_ptr<uint8_t[]> dst_ref(new uint8_t[width * height * 3 / 2]);
    std::unique_ptr<uint8_t[]> dst(new uint8_t[width * height * 3 / 2]);
    tinycv::debug::randomFill<uint8_t>(src.get(), width * height * 4, 0, 255);

    cv::Mat srcMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 4), src.get());
    cv::Mat dstMat(3 * height / 2, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 1), dst_ref.get());
    if (mode == RGBA2I420_MODE) {
        tinycv::RGBA2I420<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_RGBA2YUV_I420);
    } else if (mode == RGBA2YV12_MODE) {
        tinycv::RGBA2YV12<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_RGBA2YUV_YV12);
    } else if (mode == BGRA2I420_MODE) {
        tinycv::BGRA2I420<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_BGRA2YUV_I420);
    } else if (mode == BGRA2YV12_MODE) {
        tinycv::BGRA2YV12<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
...
void ColorAlpha2YUV420MultiPlaneTest(int32_t height, int32_t width)
{
    ...
    cv::Mat dstMat(3 * height / 2, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 1), dst_ref.get());
    if (mode == RGBA2I420_MODE) {
        tinycv::RGBA2I420<uint8_t>(height, width, width * 4, src.get(), width, dst.get(), width / 2, dst.get() + height * width, width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat, cv::COLOR_RGBA2YUV_I420);
    } else if (mode == BGRA2I420_MODE) {
        tinycv::BGRA2I420<uint8_t>(height, width, width * 4, src.get(), width, dst.get(), width / 2, dst.get() + height * width, width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat, cv::COLOR_BGRA2YUV_I420);
    }
    checkResult<uint8_t, 1>(dst.get(), dst_ref.get(), 3 * height / 2, width, width, width, 1.01f);
}
...
void YUV4202ColorTest(int32_t height, int32_t width)
{
    ...
    if (mode == I4202RGB_MODE) {
        tinycv::I4202RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_I420);
    } else if (mode == I4202BGR_MODE) {
        tinycv::I4202BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_I420);
    } else if (mode == YV122RGB_MODE) {
        tinycv::YV122RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_YV12);
    } else if (mode == YV122BGR_MODE) {
        tinycv::YV122BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
...
void YUV4202ColorMultiPlaneTest(int32_t height, int32_t width)
{
    ...
    cv::Mat dstMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 3), dst_ref.get());
    if (mode == I4202RGB_MODE) {
        tinycv::I4202RGB<uint8_t>(height, width, width, src.get(), width / 2, src.get() + height * width, width / 2, src.get() + height * width + (height / 2) * (width / 2), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_I420);
    } else if (mode == I4202BGR_MODE) {
        tinycv::I4202BGR<uint8_t>(height, width, width, src.get(), width / 2, src.get() + height * width, width / 2, src.get() + height * width + (height / 2) * (width / 2), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_I420);
    }
    checkResult<uint8_t, 1>(dst.get(), dst_ref.get(), height, width, 3 * width, 3 * width, 1.01f);
}
...
void YUV4202ColorAlphaTest(int32_t height, int32_t width)
{
    ...
    if (mode == I4202RGBA_MODE) {
        tinycv::I4202RGBA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGBA_I420);
    } else if (mode == I4202BGRA_MODE) {
        tinycv::I4202BGRA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGRA_I420);
    } else if (mode == YV122RGBA_MODE) {
        tinycv::YV122RGBA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGBA_YV12);
    } else if (mode == YV122BGRA_MODE) {
        tinycv::YV122BGRA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
...
void YUV4202ColorAlphaMultiPlaneTest(int32_t height, int32_t width)
{
    ...
    cv::Mat dstMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 4), dst_ref.get());
    if (mode == I4202RGBA_MODE) {
        tinycv::I4202RGBA<uint8_t>(height, width, width, src.get(), width / 2, src.get() + height * width, width / 2, src.get() + height * width + (height / 2) * (width / 2), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGBA_I420);
    } else if (mode == I4202BGRA_MODE) {
        tinycv::I4202BGRA<uint8_t>(height, width, width, src.get(), width / 2, src.get() + height * width, width / 2, src.get() + height * width + (height / 2) * (width / 2), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGRA_I420);
    }
    checkResult<uint8_t, 1>(dst.get(), dst_ref.get(), height, width, 4 * width, 4 * width, 1.01f);
}
