...
void Color2GRAYTest(int32_t height, int32_t width)
{
    ...
    if (nc == 3) {
        if (mode == BGR2GRAY_MODE) {
            tinycv::BGR2GRAY<T>(height, width, width * nc, src.get(), width, dst.get());
            cv::cvtColor(srcMat, dstMat, cv::COLOR_BGR2GRAY);
        }
        if (mode == RGB2GRAY_MODE) {
            tinycv::RGB2GRAY<T>(height, width, width * nc, src.get(), width, dst.get());
            cv::cvtColor(srcMat, dstMat, cv::COLOR_RGB2GRAY);
        }
    } else if (nc == 4) {
        if (mode == BGR2GRAY_MODE) {
            tinycv::BGRA2GRAY<T>(height, width, width * nc, src.get(), width, dst.get());
            cv::cvtColor(srcMat, dstMat, cv::COLOR_BGRA2GRAY);
        }
        if (mode == RGB2GRAY_MODE) {
            tinycv::RGBA2GRAY<T>(height, width, width * nc, src.get(), width, dst.get());
...
void GRAY2ColorTest(int32_t height, int32_t width)
{
    ...
    tinycv::debug::randomFill<T>(src.get(), width * height, 0, 255);
    ...
    if (nc == 3) {
        if (mode == GRAY2BGR_MODE) {
            tinycv::GRAY2BGR<T>(height, width, width, src.get(), width * nc, dst.get());
            cv::cvtColor(srcMat, dstMat, cv::COLOR_GRAY2BGR);
        }
        if (mode == GRAY2RGB_MODE) {
            tinycv::GRAY2RGB<T>(height, width, width, src.get(), width * nc, dst.get());
            cv::cvtColor(srcMat, dstMat, cv::COLOR_GRAY2RGB);
        }
    } else if (nc == 4) {
        if (mode == GRAY2BGR_MODE) {
            tinycv::GRAY2BGRA<T>(height, width, width, src.get(), width * nc, dst.get());
            cv::cvtColor(srcMat, dstMat, cv::COLOR_GRAY2BGRA);
        }
        if (mode == GRAY2RGB_MODE) {
            tinycv::GRAY2RGBA<T>(height, width, width, src.get(), width * nc, dst.get());
